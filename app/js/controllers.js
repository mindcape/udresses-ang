        ...
        $http.jsonp(furl)
          .success(function(data) {
            ...
            angular.forEach($scope.products,function(dress){
                try{
                var photo = $filter('filter')(photos,{title:dress.flikid},true)[0];
                dress.image = "https://farm"+photo.farm+".staticflickr.com/"+photo.server+"/"+photo.id+"_"+photo.secret+"_b.jpg";
                }catch(e){console.log("Error in find the image:"+e)}
            })
        })
      } catch(e){ console.log("Error looking up flickr data:"+e)};
      ...
      $scope.setPage = function (pageNo) {
       ...
       if($stateParams.anc){
         if($stateParams.currentPage){
           $scope.setPage($stateParams.currentPage)
         }
         $timeout(function() {
           if($stateParams.anc != 1) {
  ...
  function($scope, $http, $filter,loadService, detailsService, $uibModal, $stateParams,$location,$state) {
    ...
    detailsService.getItem($stateParams.id,$stateParams.type).then(function(item) {
        ...
        $http.jsonp(furl).success(function(data) {
          var photo = $filter('filter')(data.photoset.photo,{title:$stateParams.id},true)[0];
    ...
    $scope.go_back = function(path,divId) {
          var page = $state.params.page;
           $state.transitionTo(path, {type:path, anc:divId,currentPage:page },{reload:true});
    };
